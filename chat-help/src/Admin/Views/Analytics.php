            ...
            array(
                ...
                'fields' => array(
                    array(
                        ...
                        'default' => true,
                        'title_help' =>
                        '<div class="chat-help-info-label">' . esc_html__('Enable tracking of WhatsApp button clicks and interactions in Google Analytics.', 'chat-help') . '</div>' . ' <a class="tooltip_btn_primary" target="_blank" href="' . esc_url(CHAT_HELP_DEMO_URL . 'docs/analytics/?ref=1') . '">' . esc_html__('Open Docs', 'chat-help') . '</a>',
                    ),
                    array(
                        'id'      => 'event_name',
                        'type'    => 'text',
                        ...
                        'title_help' =>
                        '<div class="chat-help-info-label">' . esc_html__('Set a custom event name for tracking in Google Analytics..', 'chat-help') . '</div>' . ' <a class="tooltip_btn_primary" target="_blank" href="' . esc_url(CHAT_HELP_DEMO_URL . 'docs/analytics/?ref=1') . '">' . esc_html__('Open Docs', 'chat-help') . '</a>',
                    ...
                    array(
                        ...
                        'title_help' =>
                        '<div class="chat-help-info-label">' . esc_html__('Define additional parameters to send with your Google Analytics events. Supports variables, URL parameters, and cookies.', 'chat-help') . '</div>' . ' <a class="tooltip_btn_primary" target="_blank" href="' . esc_url(CHAT_HELP_DEMO_URL . 'docs/analytics/?ref=1') . '">' . esc_html__('Open Docs', 'chat-help') . '</a>',
                        'desc' => wp_kses_post('<div style="margin-bottom: 10px;"><b>' . esc_html__('Variables:', 'chat-help') . '</b> ' . esc_html__('Use {number}, {group}, {title}, {currentTitle}, {url} to insert the assigned number/group, page title, current title, or current URL.', 'chat-help') . '</div><h4 class="chat-help-info-label" style="margin-bottom: 10px;">' . esc_html__('Retrieving Cookies & URL Parameters', 'chat-help') . '</h4><div style="margin-bottom: 10px;"><b>' . esc_html__('URL Parameters:', 'chat-help') . '</b> ' . esc_html__('Wrap the parameter name in single square brackets [ ] to insert its value. Missing parameters return blank.', 'chat-help') . '</b><br />' . esc_html__('Example:', 'chat-help') . ' [gclid], [utm_source]</div><div style="margin-bottom: 10px;"><b>' . esc_html__('Cookies:', 'chat-help') . '</b> ' . esc_html__('Wrap the cookie name in double square brackets [[ ]] to insert its value. Missing cookies return blank.', 'chat-help') . '<br />' . esc_html__('Example:', 'chat-help') . ' [[_ga]]</div>'),
                        'fields'  => array(
                            array(
                                'id'    => 'parameter_label',
                                'type'  => 'text',
